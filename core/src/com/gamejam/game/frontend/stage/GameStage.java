...
public class GameStage extends Stage{
        ...
        for (int j = 0; j < numRows; j++) {
            ...
            for (int i = 0; i < numColumns; i++) {
                ...
                if (pieceTextureRegion != null) {
                    ...
                    if(gameBoardArray[i][j].getIsDame()){
                        // adds a crown to the piece if it is a dame
                        Image dameCrown = new Image(new Texture("misc/crown.png"));
                        dameCrown.setSize(tileSize, tileSize);
                        tileWidget.add(dameCrown);
                    }
                } else {
                ...
                if (!(gameBoardArray[i][j].getPieceType() == null)) {
                    tileWidget.addListener(new DragListener() {
                        ...
                        public void dragStart(InputEvent event, float x, float y, int pointer) {
                            ...
                            if(gameBoardArray[finalI][finalJ].getPieceType() == Piece.STONE){
                                event.cancel();
    ...
    * Method to take a piece and return the class attribute loaded Texture of that piece
    ...
    private TextureRegion getWhitePieceTexture(Piece pieceType) {
        switch (pieceType) {
            case CHECKERSTONE:
                return new TextureRegion(whiteStone);
            default:
    ...
    private TextureRegion getBlackPieceTexture(Piece pieceType) {
        switch (pieceType) {
            case CHECKERSTONE:
                return new TextureRegion(blackStone);
            default:
                Gdx.app.log("GameStage", "NO valid piece type -> Empty");
                return new TextureRegion(empty);
        }
    ...
    private void loadAllAssets() {
        ...
        // white pieces ----------------------------------------------------
        whiteStone = new Texture("white/whiteStone.png");


        // black pieces ----------------------------------------------------
        blackStone = new Texture("black/blackStone.png");
